...
/* MSCHANGE start */
/* The PWM boundary is DBV = 0x280
 * After luminance calibration, the brightness level needs to be adjusted
 * according to the LUT2 calibration table.
 * Brightness = (640*255)/2047 = ~80
 */
#define MS_BL_MARGIN_HIGH	80
/* MSCHANGE end */
...
static int sde_backlight_device_update_status(struct backlight_device *bd)
{
	...
	/* MSCHANGE start */
	/* Prevent idle pc when brightness slider is <= 30%.
	 * Idle pc is restored when in sleep (brightness = 1)
	 * and when brightness > 30%.
	 */
	if (brightness <= MS_BL_MARGIN_HIGH &&
			brightness > display->panel->bl_config.bl_min_level)
		sde_encoder_control_idle_pc(c_conn->encoder, false);
	else
		sde_encoder_control_idle_pc(c_conn->encoder, true);
	/* MSCHANGE end */
