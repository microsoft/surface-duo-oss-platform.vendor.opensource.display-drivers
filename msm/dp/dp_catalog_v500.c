		...
		struct dp_aux_cfg *cfg)
{
	...
	dp_write(catalog->exe_mode, io_data, DP_PHY_PD_CTL_V500, 0x7d);
	...
	dp_write(catalog->exe_mode, io_data, DP_PHY_AUX_INTERRUPT_MASK_V500,
			0x1F);
}
...
static void dp_catalog_aux_clear_hw_interrupts_v500(struct dp_catalog_aux *aux)
{
	...
	data = dp_read(catalog->exe_mode, io_data,
		DP_PHY_AUX_INTERRUPT_STATUS_V500);

	dp_write(catalog->exe_mode, io_data,
		DP_PHY_AUX_INTERRUPT_CLEAR_V500, 0x1f);
	wmb(); /* make sure 0x1f is written before next write */
	dp_write(catalog->exe_mode, io_data,
		DP_PHY_AUX_INTERRUPT_CLEAR_V500, 0x9f);
	wmb(); /* make sure 0x9f is written before next write */
	dp_write(catalog->exe_mode, io_data,
		DP_PHY_AUX_INTERRUPT_CLEAR_V500, 0);
					...
					u32 rate, u32 stream_rate_khz)
{
	...
	default:
		...
		if (panel->stream_id == DP_STREAM_1)
			reg_off = MMSS_DP_PIXEL1_M_V500 - MMSS_DP_PIXEL_M_V500;
		break;
	case 1:
		/* DP controller 1 */
		if (panel->stream_id == DP_STREAM_0)
			reg_off = MMSS_DP_PIXEL2_M_V500 - MMSS_DP_PIXEL_M_V500;
		else
			reg_off = MMSS_DP_PIXEL1_M_V500 - MMSS_DP_PIXEL_M_V500;
		break;
	case 2:
		...
		reg_off = MMSS_EDP_PIXEL_M_V500 - MMSS_DP_PIXEL_M_V500;
	}

	pixel_m = dp_read(catalog->exe_mode, io_data,
			MMSS_DP_PIXEL_M_V500 + reg_off);
	pixel_n = dp_read(catalog->exe_mode, io_data,
			MMSS_DP_PIXEL_N_V500 + reg_off);
		...
		bool flipped, u8 ln_cnt)
{
	...
	dp_write(catalog->exe_mode, io_data, DP_PHY_SPARE0_V500, info);
}

static void dp_catalog_ctrl_set_phy_bond_mode_v500(struct dp_catalog_ctrl *ctrl,
		enum dp_phy_bond_mode phy_bond_mode)
{
	...
	info = dp_read(catalog->exe_mode, io_data, DP_PHY_SPARE0_V500);
	info &= 0x3F;
	info |= ((bond & 0x03) << 6);
	pr_debug("Shared Info = 0x%x\n", info);
		...
		u8 v_level, u8 p_level, bool high)
{
	...
	dp_write(catalog->exe_mode, io_data, TXn_LDO_CONFIG_V500, 0x01);
	dp_write(catalog->exe_mode, io_data, TXn_TX_DRV_LVL_V500, 0x17);
	dp_write(catalog->exe_mode, io_data, TXn_TX_EMP_POST1_LVL_V500, 0x00);

	io_data = catalog->io->dp_ln_tx1;
	dp_write(catalog->exe_mode, io_data, TXn_LDO_CONFIG_V500, 0x01);
	dp_write(catalog->exe_mode, io_data, TXn_TX_DRV_LVL_V500, 0x2A);
	dp_write(catalog->exe_mode, io_data, TXn_TX_EMP_POST1_LVL_V500, 0x20);

	/* Configure host and panel only if both values are allowed */
	if (value0 != 0xFF && value1 != 0xFF) {
		io_data = catalog->io->dp_ln_tx0;
		dp_write(catalog->exe_mode, io_data, TXn_LDO_CONFIG_V500,
				ldo_cfg);
		dp_write(catalog->exe_mode, io_data, TXn_TX_DRV_LVL_V500,
				value0);
		dp_write(catalog->exe_mode, io_data, TXn_TX_EMP_POST1_LVL_V500,
				value1);
		...
		pr_debug("hw: vx_value=0x%x px_value=0x%x ldo_value=0x%x\n",
						...
						u8 ln_pnswap)
{
	...
	io_data = catalog->io->dp_ln_tx0;
	dp_write(catalog->exe_mode, io_data, TXn_TX_POL_INV_V500, cfg0);

	io_data = catalog->io->dp_ln_tx1;
	dp_write(catalog->exe_mode, io_data, TXn_TX_POL_INV_V500, cfg1);
}
...
static void dp_catalog_set_exe_mode_v500(struct dp_catalog *catalog,
		char *mode)
{
	struct dp_catalog_private_v500 *catalog_priv;
		...
		void *io)
{
	...
	catalog->priv.put          = dp_catalog_put_v500;
	catalog->priv.set_exe_mode = dp_catalog_set_exe_mode_v500;
	...
	catalog->ctrl.set_phy_bond_mode =
				dp_catalog_ctrl_set_phy_bond_mode_v500;
	...
	dp_catalog_set_exe_mode_v500(catalog, "hw");
