...
int dp_sim_update_port_num(struct msm_dp_aux_bridge *bridge, u32 port_num)
{
	...
	rc = msm_dp_mst_sim_update(sim_dev->bridge.mst_ctx,
		...
		int port, enum drm_connector_status status)
{
	...
	return msm_dp_mst_sim_update(sim_dev->bridge.mst_ctx,
...
{
	struct dp_sim_device *sim_dev;
	struct msm_dp_mst_sim_port *sim_port;
...
static int dp_sim_debug_init(struct dp_sim_device *sim_dev)
{
	...
	for (i = 0; i < sim_dev->port_num; i++) {
		...
		if (IS_ERR_OR_NULL(file)) {
			rc = PTR_ERR(file);
			pr_err("[%s] debugfs create hpd failed, rc=%d\n",
					sim_dev->label, rc);
			goto error_remove_dir;
		}
	}
...
int dp_sim_create_bridge(struct device *dev, struct msm_dp_aux_bridge **bridge)
{
