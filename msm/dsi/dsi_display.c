...
#define DSI_DISPLAY_PRIMARY_FACTORY_CRS "qcom,mdss_dsi_lucinda_dsi0_c3_cmd"
#define DSI_DISPLAY_SECONDARY_FACTORY_CRS "qcom,mdss_dsi_lucinda_dsi1_r2_cmd"
...
static void dsi_display_resolve_display_boot_mode()
{
	...
	uint16_t product_id = get_sproduct_id();
	...
	if (tiled_sync_display)
		...
		if (!!strnstr(dsi_display_primary, "ev2", prim_len))
			...
			// Only point to CRS FF here since CRS PD defaults to Elgin DV
			if (OEMC2 == product_id)
				snprintf(ver, sizeof(ver), "_ev1");
			else
				snprintf(ver, sizeof(ver), "_mp");
		}
		...
		// Only point to CRS FF here since CRS PD defaults to Elgin DV
		if (OEMC2 == product_id) {
			factory_prim_str = DSI_DISPLAY_PRIMARY_FACTORY_CRS;
			factory_sec_str = DSI_DISPLAY_SECONDARY_FACTORY_CRS;
		}
